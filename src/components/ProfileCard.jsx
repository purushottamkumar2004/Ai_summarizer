...
import Swapbtn from "../Buttons/Swapbtn";
import { useAuth0 } from '@auth0/auth0-react'
const ProfileCard = ({plan,name}) => {
    const style1 ={
        fill:'none',
        strokeLinecap:"round",
        stroke:"#fff",
  ...
  return (
    <div>
      ...
      {user.picture ? (
        <img src={user.picture} className="rounded-full" alt="" />
      ) : (
        ...
        <div class="Pcard__title">{name}</div>
        <div class="Pcard__subtitle">{plan}</div>
        <div class="Pcard__wrapper flex  mt-5 gap-5">
          <Swapbtn name={"Upgrade"} link={"/Pricing"} />
        <Swapbtn name ={"Contact"} link={"/Contact"} />
        </div>
      </div>
    </div>
  );
